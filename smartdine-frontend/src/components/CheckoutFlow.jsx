...
export default function CheckoutFlow({ onClose, isHost, userId, refreshTrigger }) {
  ...
  const handleSetMethod = async (method) => {
    try {
      ...
      // Wait for socket to trigger refresh
    } catch (err) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // Wait for socket `session_closed` to redirect
    } catch (err) {
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col justify-center relative bg-white">
          ...
          {!hasSelectedMethod ? (
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              <h3 className="text-2xl font-bold text-slate-800 mb-2">How would you like to pay?</h3>
              
              {isHost ? (
                <>
                  <p className="text-slate-500 text-sm mb-6">Select a split method for the table.</p>
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col h-full">
              <div className="mb-8">
                ...
                {bill.paymentSplitMethod === 'itemized' && bill.itemizedByUser[userId] && (
                  <p className="text-xs text-slate-500 font-medium">Includes ${(bill.itemizedByUser[userId].taxShare).toFixed(2)} tax on your items.</p>
                )}
              </div>

              {amIPaid || paymentSuccess || myShareAmount === 0 ? (
                ...
                <div className="mt-auto">
                  ...
                  >
                    ...
                    <span>{processingPayment ? 'Processing...' : 'Pay with Apple Pay'}</span>
                  </button>
                </div>
              )}
            </div>
